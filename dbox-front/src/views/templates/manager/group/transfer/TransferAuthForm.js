...
import DeptTransferFormContainer from "views/containers/manager/group/transfer/dept/DeptTransferFormContainer";

console.log("TransferAuthForm.js");
...
export default function TransferAuthForm({ result }) {
          ...
          {result
            ? result.map((item, index) => (
                <TabPanel value={value} index={index}>
                  <Box p={3}>{item.code.uCodeName3 === "DeptTransferFormContainer" ? <DeptTransferFormContainer /> : null}</Box>
                </TabPanel>
              ))
            : null}
        </div>
