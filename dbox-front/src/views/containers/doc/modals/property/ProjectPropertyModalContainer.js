...
console.debug("ProjectPropertyModalContainer.js");
...
const ProjectPropertyModalContainer = forwardRef(function (props, ref) {
  ...
  const openModal = async (pjtCode) => {
    ...
    if (pjtCode) {
      try {
        ...
        // 현재 프로젝트/투자 정보 세팅
        setProperty({
          pjtCode: response.data.response.uPjtCode,
  ...
  const saveProperty = (isApply) => {
    ...
    openDialog(CONFIRM_DIALOG, {
      title: "확인",
      content: "프로젝트 속성을 변경하시겠습니까?",
      onOkClick: async () => {
        // 프로젝트/투자 수정
        await requestProjectModify();
   ...
   * 프로젝트/투자 수정
   */
  const requestProjectModify = async () => {
    try {
      await ProjectApi.patchProject({
        params: {
          uPjtCode: property.pjtCode,
...
export default ProjectPropertyModalContainer;
