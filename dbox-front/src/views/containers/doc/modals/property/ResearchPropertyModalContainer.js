...
console.debug("ResearchPropertyModalContainer.js");
...
const ResearchPropertyModalContainer = forwardRef(function (props, ref) {
  ...
  const openModal = async (rschCode) => {
    ...
    if (rschCode) {
      try {
        ...
        // 현재 연구과제 정보 세팅
        setProperty({
          rschCode: response.data.response.uRschCode,
  ...
  const saveProperty = (isApply) => {
    ...
    openDialog(CONFIRM_DIALOG, {
      title: "확인",
      content: "연구과제 속성을 변경하시겠습니까?",
      onOkClick: async () => {
        // 프로젝트/투자 수정
        await requestResearchModify();

        // 모달, 확인창 닫기
        if (!isApply) window.location.reload();
        closeDialog();
      },
    });
  };
   ...
   * 연구과제 수정
   */
  const requestResearchModify = async () => {
    try {
      await ResearchApi.patchResearch({
        params: {
          uRschCode: property.rschCode,
...
export default ResearchPropertyModalContainer;
